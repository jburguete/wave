...
void dialog_wave_create()
{
	...
	dlg->frame_shape_initial
		= (GtkFrame*)gtk_frame_new(gettext("Initial shape"));
	...
	gtk_grid_attach(dlg->table,
		(GtkWidget*)dlg->box_frames, 0, 0, 1, 7);
	gtk_grid_attach(dlg->table,
	...
	gtk_grid_attach(dlg->table,
		(GtkWidget*)dlg->label_shape_base, 1, 3, 1, 1);
	gtk_grid_attach(dlg->table,
		(GtkWidget*)dlg->entry_shape_base, 2, 3, 1, 1);
	gtk_grid_attach(dlg->table,
