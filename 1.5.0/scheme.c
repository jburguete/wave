...
JBDOUBLE cfl, o, pv, dxmax = 0.L, initial_mass, final_mass;
...
#if JBW != JBW_NO
int mesh_cells = 0, dialog_convergence = 0, max_cells = 0,
  convergence_number = 0;
JBDOUBLE simulation_time = 0.L, cfl_number = 0.L, implicit = 0.L, viscosity =
  0.L;
extern void dialog_simulator_update ();
#endif
...
void (*step_internal) (Parameters *, void (*)(Parameters *),
...
variables_start ()
{
  ...
  if (!p)
    jbw_show_error (_("No enough memory to start the variables"));
  while (--i >= 0)
    for (j = 4; --j >= 0;)
      p[i].BB[j] = p[i].AA[j] = 0.L;
#if DEBUG_VARIABLES_START
      ...
      for (i = 0; ++i < (n - 1);)
        {
          ...
          p[i].dxx += k * (p[i].ix * (k3 + p[i + 1].ix) + 2.L * p[i - 1].ix *
                           (2.L * k3 + p[i + 1].ix)) / (p[i - 1].ix * k3);
          p[i + 1].dxx +=
            k * (p[i].ix * (k2 + p[i - 1].ix) +
                 2.L * p[i + 1].ix * (2.L * k2 + p[i - 1].ix))
            / (p[i + 1].ix * k2);
...
static inline void
parameters_cell_base_finite_volumes (Parameters * p)
{
...
static inline void
parameters_cell_base_finite_differences (Parameters * p)
{
...
void
parameters_cell_TVD_finite_volumes (Parameters * p)
{
  ...
  if (p->a > 0.L)
    p->Lp = p->dF / p->ix, p->Lm = 0.L;
  else
    p->Lm = p->dF / p->ix, p->Lp = 0.L;
#if DEBUG_PARAMETERS_CELL_TVD_FINITE_VOLUMES
...
void
parameters_cell_TVD_finite_differences (Parameters * p)
{
...
void
parameters_cell_TVD2_finite_volumes (Parameters * p)
{
...
void
parameters_cell_TVD2_finite_differences (Parameters * p)
{
...
#pragma omp parallel for default(none) \
  shared(p,n,discretization_type,k,propagation,parameters_cell,stderr) \
  private(i)
*/
  for (i = n - 1; i >= 0; --i)
    {
      ...
      parameters_cell (p + i);
      if (p[i].a < 0.L)
        k = fmaxl (k, -p[i].a / p[i].dx);
      else
        k = fmaxl (k, p[i].a / p[i + 1].dx);
#if DEBUG_PARAMETERS_ALL
...
JBDOUBLE
entropy_correction_viscosity_no (Parameters * p)
{
  return 0.L;
}

JBDOUBLE
entropy_correction_viscosity_Harten (Parameters * p)
{
  return 0.5L * ((p + 1)->c - p->c);
}

JBDOUBLE
entropy_correction_viscosity_mean (Parameters * p)
{
  return 0.5L * fminl ((p + 1)->c, -p->c);
}

JBDOUBLE
entropy_correction_viscosity_mean_mean (Parameters * p)
{
  return 0.25L * ((p + 1)->c - p->c - fabs ((p + 1)->c + p->c));
}

JBDOUBLE
entropy_correction_viscosity_interpolated (Parameters * p)
{
...
JBDOUBLE
entropy_correction_viscosity_interpolated_mean (Parameters * p)
{
  register JBDOUBLE k, kp, km;
  if (p->a > 0.L)
    kp = p->a, km = 0.L;
...
void
entropy_correction (Parameters * p)
{
  register JBDOUBLE k;
...
void
entropy_correction_implicit (Parameters * p)
{
  register JBDOUBLE k;
...
void
viscosity_LaxFriedrichs (Parameters * p)
{
  register JBDOUBLE k;
  k = 0.5L * pv * p->au;
  (p + 1)->iu -= k;
...
void
viscosity_LaxFriedrichs_optimized (Parameters * p)
{
  register JBDOUBLE k;
  k = 0.5L * dt * fmaxl (fabsl (p->c), fabsl ((p + 1)->c)) * p->au / p->ix;
  (p + 1)->iu -= k;
  p->iu += k;
}

void
waves_centered (Parameters * p)
{
  p->AA[1] = p->AA[2] = 0.5L;
}

void
waves_right (Parameters * p)
{
  p->AA[2] = 1.L;
}

void
waves_left (Parameters * p)
{
  p->AA[1] = 1.L;
}

void
waves_upwind (Parameters * p)
{
  if (p->a > 0.L)
    p->AA[1] = 1.L, p->AA[2] = 0.L;
  else
    p->AA[2] = 1.L, p->AA[1] = 0.L;
}

void
waves_LaxWendroff (Parameters * p)
{
  p->AA[1] = p->AA[2] = 0.5L;
  p->BB[2] = 1.L;
  p->BB[1] = -1.L;
...
void
waves_WarmingBeam (Parameters * p)
{
  if (p->a > 0.L)
    {
      p->AA[0] = -0.5L;
      ...
      p->BB[1] = 1.L;
      p->BB[0] = -1.L;
      p->AA[2] = p->AA[3] = p->BB[2] = p->BB[3] = 0.L;
    }
  else
    {
      p->AA[3] = -0.5L;
      ...
      p->BB[3] = 1.L;
      p->BB[2] = -1.L;
      p->AA[0] = p->AA[1] = p->BB[0] = p->BB[1] = 0.L;
    }
}

void
waves_upwind2 (Parameters * p)
{
  if (p->a > 0.L)
    {
      p->AA[0] = -0.5L * (p + 1)->ix / p->ix;
      p->AA[1] = 1.L - p->AA[0];
      p->BB[1] = 1.L;
      p->BB[0] = -1.L;
      p->AA[2] = p->AA[3] = p->BB[2] = p->BB[3] = 0.L;
...
void
waves_TVD_classical (Parameters * p)
{
  if (p->a > 0.L)
    {
      p->AA[0] = -0.5L * limiter ((p + 1)->Lp, p->Lp);
      p->AA[1] = 1.L - p->AA[0];
      p->AA[2] = p->AA[3] = 0.L;
    }
  else
    {
      p->AA[3] = -0.5L * limiter ((p - 1)->Lm, p->Lm);
      p->AA[2] = 1.L - p->AA[3];
      p->AA[0] = p->AA[1] = 0.L;
    }
}
...
void
waves_TVD (Parameters * p)
{
  if (p->a > 0.L)
    {
      p->AA[0] = -0.5L * limiter ((p + 1)->Lp, p->Lp) * (p + 1)->ix / p->ix;
      p->AA[1] = 1.L - p->AA[0];
      p->AA[2] = p->AA[3] = 0.L;
    }
  else
    {
      p->AA[3] = -0.5L * limiter ((p - 1)->Lm, p->Lm) * (p - 1)->ix / p->ix;
      p->AA[2] = 1.L - p->AA[3];
      p->AA[0] = p->AA[1] = 0.L;
    }
}
...
void
waves_TVD_RoeSweby (Parameters * p)
{
  register JBDOUBLE k;
  if (p->a > 0.L)
    {
...
void
waves_upwind_implicit (Parameters * p)
{
  register JBDOUBLE k;
  k = dt * p->dF;
  if (p->a > 0)
...
void
waves_right_implicit (Parameters * p)
{
  p->ium -= dt * p->dF;
}

void
waves_left_implicit (Parameters * p)
{
  (p + 1)->iup -= dt * p->dF;
}

void
semi_explicit_parameters (Parameters * p)
{
  int i;
  JBDOUBLE k;
  ...
  if (discretization_type)
    {
      ...
      for (i = 0; ++i <= n;)
        {
          k = fmaxl (fmaxl (k * p[i - 1].dxx / p[i].dxx,
                            p[i - 1].a * dt / p[i].dxx) - 1.L, 0.L);
          p[i].Ep = k;
#if DEBUG_SEMI_EXPLICIT_PARAMETERS
      ...
      while (--i >= 0)
        {
          k =
            fminl (fminl (k * p[i + 1].dxx / p[i].dxx, p[i].a * dt / p[i].dxx)
                   + 1.L, 0.L);
          p[i].Em = k;
#if DEBUG_SEMI_EXPLICIT_PARAMETERS
...
void
semi_explicit_waves (Parameters * p)
{
  p->Dp = 1.L + p->Ep;
  p->C = -p->Ep;
  p->Dm = 1.L + p->Em;
...
void
implicit_waves_upwind (Parameters * p)
{
  register JBDOUBLE k;
  k = o * p->c * dt / p->dxx;
  p->Dp = p->Dm = 1.L;
...
void
implicit_waves_upwind_optimized (Parameters * p)
{
  register JBDOUBLE k;
  k = p->c * dt / p->dxx;
  p->Dp = p->Dm = 1.L;
...
void
implicit_waves_right (Parameters * p)
{
  register JBDOUBLE k;
...
void
implicit_waves_left (Parameters * p)
{
  register JBDOUBLE k;
  k = o * fabs (p->c) * dt / p->dxx;
  p->Dp = p->Dm = 1.L;
...
void
scheme_explicit_inlet (Parameters * p)
{
  register JBDOUBLE k, k2;
  viscosity_artificial (p);
  k = dt * p->dF;
...
void
scheme_explicit_outlet (Parameters * p)
{
  register JBDOUBLE k, k2;
  viscosity_artificial (p);
  k = dt * p->dF;
  ...
  if (k >= 0.L || p[0].c >= 0.L)
    {
      ...
      if (!boundary_type)
        return h - u;
      return p[0].iu + (yy0 (t) - yy0 (t - dt) -
                        p[0].u * p[0].c * dt) / p[0].dx;
    }
  fix_in = 0;
#if DEBUG_INLET
...
#endif
  for (i = 0; ++i <= n;)
    p[i].iup = (p[i].iup - p[i - 1].C * p[i - 1].iup) / p[i].Dp;
  for (i = n; --i >= 0;)
    p[i].ium = (p[i].ium - p[i + 1].E * p[i + 1].ium) / p[i].Dm;
...
void
solve_tridiagonal (Parameters * p)
{
  register int i;
...
void
step_explicit (Parameters * p,
               void (*waves) (Parameters *),
               void (*waves_inlet) (Parameters *),
               void (*waves_outlet) (Parameters *))
{
  register int i;
  register JBDOUBLE k;
...
#if DEBUG_STEP_EXPLICIT
  for (i = 0; i <= n; ++i)
    fprintf (stderr, "SE i=%d iu=" FWL "\n", i, p[i].iu);
  fprintf (stderr, "step_explicit: end\n");
#endif
}

void
step_implicit (Parameters * p, void (*waves) (Parameters *),
               void (*implicit_waves) (Parameters *),
...
void
step_semi_explicit (Parameters * p,
                    void (*waves) (Parameters *),
                    void (*implicit_waves) (Parameters *),
                    void (*unused) (Parameters *))
{
...
scheme_start ()
{
  ...
  if (discretization_type)
    ...
    parameters_cell = parameters_cell_base_finite_differences;
  ...
  switch (scheme_type)
    ...
    case 7:
      if (discretization_type)
        ...
        parameters_cell = parameters_cell_TVD_finite_differences;
    ...
    case 9:
      if (discretization_type)
        ...
        parameters_cell = parameters_cell_TVD2_finite_differences;
  ...
  switch (entropy_type)
    ...
    case 3:
      ...
      break;
    case 4:
      entropy_correction_viscosity = entropy_correction_viscosity_interpolated;
      break;
    default:
...
#endif
  for (i = 0, initial_mass = 0.L; i <= n; ++i)
    initial_mass += p[i].u * p[i].dx;
  while (t < tf && simulating)
    {
      tmax = tf;
      step ();
      for (i = 0, final_mass = 0.L; i <= n; ++i)
        final_mass += p[i].u * p[i].dx;
      ++nsteps;
      if (!convergence)
        draw ();
#if JBW != JBW_NO
...
execute (char *file_name)
{
  ...
  if (convergence)
    {
      r1 = n;
      r2 = powl (nmax / r1, 1.L / (JBDOUBLE) nc);
