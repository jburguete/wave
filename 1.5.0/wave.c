...
JBFLOAT wave_velocity, shape_position, shape_width, shape_amplitude,
...
JBDOUBLE
propagation_linear (JBDOUBLE u)
{
  return wave_velocity;
}

JBDOUBLE
propagation_Burgers (JBDOUBLE u)
{
  return u;
}
...
y_adjust ()
{
  ...
  if (wave_type == 0)
    ...
    {
      xb = 0.5L * domain_width;
      xa = fmin (xb, shape_base * t);
      xc = domain_width - xa;
    }
  else
    {
      if (shape_amplitude > 0.L)
        p = 1.L;
      else
        p = -1.L;
      xa = shape_position + shape_base * t - 0.5L * p * shape_width;
      if (shape_initial == 0)
        {
          ...
          if (t < tt)
            {
              ...
              xc = xb + p * shape_width - 0.5L * shape_amplitude * t;
            }
          else
            {
              m = sqrtl (1.L + p * (t - tt) * shape_amplitude /
        ...
        {
          tt = 0.5L * p * shape_width / shape_amplitude;
          if (t < tt)
            {
              xb = xa + 0.5L * p * shape_width + shape_amplitude * t;
              xc = xb + p * shape_width;
            }
          else
            {
              m = sqrtl (1.L + p * (t - tt) * shape_amplitude / shape_width);
...
JBDOUBLE
yy_square (JBDOUBLE t)
{
  register JBDOUBLE k;
  if (t <= ta0)
    k = shape_base * t;
  else if (t <= tc0)
    k = shape_base * t + shape_amplitude * (t - ta0);
  else
    k = shape_base * t + shape_amplitude * (tc0 - ta0);
  return k * wave_velocity;
}

JBDOUBLE
y_triangular (JBDOUBLE x)
{
  if (x <= xa || x >= xc)
    return shape_base;
  if (x < xb)
...
yy_triangular (JBDOUBLE t)
{
  ...
  if (t <= ta0)
    ...
    k = shape_base * t +
      0.5L * (t - ta0) * jbm_extrapolate (t, ta0, tb0, 0, shape_amplitude);
  else if (t < tc0)
    k = shape_base * t + 0.5L * shape_amplitude * (tc0 - ta0) +
      0.5L * (t - tc0) * jbm_extrapolate (t, tb0, tc0, shape_amplitude, 0);
  else
    k = shape_base * t + 0.5L * shape_width * shape_amplitude;
  return k * wave_velocity;
}

JBDOUBLE
y_sinusoidal (JBDOUBLE x)
{
  if (x <= xa || x >= xc)
    return shape_base;
  return shape_base + shape_amplitude * sinl (M_PIl * (x - xa) / shape_width);
}

JBDOUBLE
yy_sinusoidal (JBDOUBLE t)
{
  register JBDOUBLE k;
  if (t <= ta0)
    k = shape_base * t;
    ...
    k = shape_base * t + shape_amplitude * (tc0 - ta0) *
      (1.L - cosl (M_PIl * (t - ta0) / (tc0 - ta0))) / M_PIl;
  else
    k = shape_base * t + 2.L / M_PIl * shape_amplitude * (tc0 - ta0);
  return k * wave_velocity;
}

JBDOUBLE
y_Gaussian (JBDOUBLE x)
{
...
*/
JBDOUBLE
yy_Gaussian (JBDOUBLE t)
{
  return (shape_base * t + 0.5L * sqrtl (M_PIl) * shape_amplitude
          * (tc0 - tb0) * (erfl ((t - tb0) / (tc0 - tb0))
                           - erfl (-tb0 / (tc0 - tb0)))) * wave_velocity;
}

JBDOUBLE
y_square_Burgers (JBDOUBLE x)
{
  if (shape_amplitude > 0.L)
    {
      if (x <= xa || x >= xc)
...
JBDOUBLE
yy0_square_Burgers (JBDOUBLE t)
{
  return 0.5L * jbm_fsqr (shape_base) * t;
}

JBDOUBLE
yyf_square_Burgers (JBDOUBLE t)
{
  return 0.5L * jbm_fsqr (shape_base) * t;
}

JBDOUBLE
y_triangular_Burgers (JBDOUBLE x)
{
  if (shape_amplitude > 0.L)
    {
      if (x <= xa || x >= xc)
...
JBDOUBLE
yy0_triangular_Burgers (JBDOUBLE t)
{
  return 0.5L * jbm_fsqr (shape_base) * t;
}

JBDOUBLE
yyf_triangular_Burgers (JBDOUBLE t)
{
  return 0.5L * jbm_fsqr (shape_base) * t;
}

JBDOUBLE
y_steady_Burgers (JBDOUBLE x)
{
  if (shape_base < 0.L)
    return 0.L;
  if (x <= xa || x >= xc)
